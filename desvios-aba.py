...
export async function GET() {
  try {
    ...
    if (!["OPERATOR", "OWNER"].includes(role)) {
      return NextResponse.json(
        { error: "Acesso restrito ao proprietário ou operador." },
        { status: 403 }
      );
    }
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      allowed: ["OPERATOR", "OWNER"].includes(role),
    });
  } catch {
    return NextResponse.json(
      { allowed: false },
...
def main():
    ...
    print("Acesso: somente OWNER ou OPERATOR")
    ...
    if sidebar:
        ...
        print("A página continua acessível em /desvios somente para OWNER ou OPERATOR.")
