...
def patch_sidebar(backup_root: Path) -> Path | None:
    for path in sidebar_candidates():
        ...
        if '"/desvios"' in source or "'/desvios'" in source:
            return path

        match = re.search(
            r'(\{[^{}]{0,350}(?:href|url|path)\s*:\s*["\']/dashboard["\'][^{}]{0,350}\})',
            source,
            re.S,
        )
        ...
        if match:
            ...
            new_item = re.sub(
                r'((?:label|title|name|text)\s*:\s*["\'])Dashboard(["\'])',
                r'\1Desvios\2',
                new_item,
                count=1,
                flags=re.I,
            ...
            if new_item != item:
                source = source[:match.end()] + ",\n" + new_item + source[match.end():]

        if source == old:
            match = re.search(
                r'(<Link\b[^>]*href=["\']/dashboard["\'][^>]*>\s*Dashboard\s*</Link>)',
                source,
                re.I,
            )
...
def main():
    ...
    if sidebar:
        ...
        print("A página continua acessível em /desvios.")
