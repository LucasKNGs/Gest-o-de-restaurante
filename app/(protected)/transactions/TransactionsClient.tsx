...
export default function TransactionsClient(){
 ...
 useEffect(()=>{load();},[]);
 async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();setError("");const form=e.currentTarget;const f=new FormData(form);const body={type:f.get("type"),description:f.get("description"),amount:f.get("amount"),occurredAt:`${f.get("occurredAt")}T12:00:00`,paymentMethod:f.get("paymentMethod"),categoryId:f.get("categoryId")||null,accountId:f.get("accountId")||null,notes:f.get("notes")||null}; const r=await fetch("/api/transactions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});if(!r.ok){const d=await r.json();return setError(d.error||"Erro");} form.reset(); await load();}
